...
##### try and do the work in the R script in bash instead

exit 0
# works fine so far, but rest is too complicated. Needes to be done in R.



# cluster high-quality tails

#test.txt #| grep -i -e "ggttttaacccagttactc" -e "ggtttaattacccaagttt"
...
# take the clusterIDs from all these candidates
# get cluster members using the cluster ID
# extract fasta and search for splicing acceptor site

## check for presence of AG splice acceptor site at genomic locations
samtools faidx $genome
bedtools bamtobed -i $outdir/01_tails.bam \
 | awk '$6=="+"{$3=$2} $6=="-"{$2=$3}{print}' OFS="\t" \
 | bedtools slop -g $genome.fai -i - -s -l 2 -r 10 \
 | bedtools getfasta -fi $genome -bed - -s -tab -name+ \
 | awk '$2~"AG"' > $outdir/03_AGsplice.txt

# link with SL cluster members
#grep -i -e "ggttttaacccagttactc" -e "ggtttaattacccaagttt" $outdir/*candidates* 
grep -o "clusterid=[0-9]*" $outdir/*candidates* \
| sed 's/clusterid=/^/g' \
| sort | uniq | LANG=C grep -w -f - $outdir/02_clusters.clusterinfo \
| cut -f 2 | LANG=C grep -F -f - $outdir/03_AGsplice.txt > $outdir/03_AGsplice_SL.txt



exit 

######### cdhit-clustering (old)
echo "$(timestamp) Clustering tails ..."
cat $outdir/01_tails.*p.fa > $outdir/01_tails.fa
# first, cluster identical tails
cd-hit-est -i $outdir/01_tails.fa -o $outdir/02_derep.fa \
	-c 1.0 -G 0 -aS 1 -aL 1 \
	-d 0 -T $threads -M 0 \
	-l 7 -g 0 -r 0 -n 8 \
	> $outdir/log_cdhitest_derep.txt 2>&1
awk '{gsub("^>|...$", "", $3)} $4=="*"{c=$3}!/^>/{print c, $3}' OFS="\t" $outdir/02_derep.fa.clstr \
	> $outdir/02_derep.fa.clusterinfo
	
	cd-hit-est -i $outdir/03_HQ_tails.fa -o $outdir/03_HQ_clusters.fa \
	-c 1.0 -G 1 \
	-d 0 -T $threads -M 0 \
	-l 7 -g 1 -p 1 -r 0 -n 8 \
	> $outdir/log_cdhitest_cluster_HQ.txt 2>&1
awk '/^>/{c=$2}!/^>/{gsub("^>|...$", "", $3); print c, $3}' OFS="\t" $outdir/03_HQ_clusters.fa.clstr \
	> $outdir/03_HQ_clusters.fa.clusterinfo
 
#############################################


########## vsearch clustering (old)
vsearch --derep_fulllength $outdir/01_tails.fa \
	--output $outdir/01_tails.fa.derep \
	--minseqlength 8 --sizeout \
	> $outdir/log_vsearch-derep.txt 2>&1
	
vsearch --cluster_fast $outdir/01_tails.fa \
	--centroids $outdir/02_clusters.centroids \
	--msaout $outdir/02_clusters.msa \
	--consout $outdir/02_clusters.cons \
	--id 1 --iddef 2 --minseqlength 8 --qmask none \
	--threads $threads --sizein --sizeout --clusterout_id \
	> $outdir/log_vsearch-cluster.txt 2>&1

# summarise cluster info
grep "^>" $outdir/02_clusters.msa | \
	awk -F';' 'BEGIN{c=0}/consensus/{c++}!/consensus/{gsub(">\\*?", "", $1); print c, $1}' OFS='\t' \
	> $outdir/02_clusters.clusterinfo
########################################################

#### tail extraction (old)
# unstranded reads: all tails are relevant				-F 20				-F 4 -f 16
# fwd-stranded (-s 1): R1 is from 5' end of transcript. -F 20	-f 64		-F 4 -f 80
# rev-stranded (-s 2): R2 is from 5' end of transcript. -F 20	-f 128		-F 4 -f 144

# reads mapping to forward strand can be taken as is (CIGAR e.g. 10S90M) #;"$3";"$4";+
samtools view -h -F 20 -f $((0+(64*$stranded))) $bam | awk '$6 ~ "^[0-9]*S"' | tee $outdir/01_tails.FWD.sam \
	| awk '{gsub("S.*", "", $6); t=substr($10,1,$6); print ">"$1"\n"t}' \
	> $outdir/01_tails.FWD.fa

# reads mapping to reverse strand must be reverse complemented (CIGAR e.g. 90M10S) #;"$3";"$4";-
samtools view -h -F 4 -f $((16+(64*$stranded))) $bam | awk '$6 ~ "[0-9]*S$"' | tee $outdir/01_tails.REV.sam \
	| awk '{gsub("S", "", $6); gsub(".*[A-Z]", "", $6); t=substr($10,length($10)+1-$6,$6); print ">"$1"\n"t}' \
	| seqtk seq -r > $outdir/01_tails.REV.fa

# make new BAM file with header	
cat <(samtools view -H $bam) $outdir/01_tails.FWD.sam $outdir/01_tails.REV.sam | samtools sort -O bam -o $outdir/01_tails.bam
cat $outdir/01_tails.FWD.fa $outdir/01_tails.REV.fa > $outdir/01_tails.fa
###################################################################
...
# check for presence of AG splice acceptor site at genomic locations
samtools faidx $genome
bedtools bamtobed -i $bam.tails.bam \
 | awk '$6=="+"{$3=$2+2} $6=="-"{$2=$3-2}{print}' OFS="\t" \
 | bedtools slop -g $genome.fai -i - -s -l 2 -r 0 \
 | bedtools getfasta -fi $genome -bed - -s -tab -name+ \
 | awk '$2~"AG"' > $bam.tails.bam.AGsplice.txt

exit



#~/sharedscratch/apps/exonerate -q cel_sl.fa -t $outdir/tails.fwd.fa -n 1 --score 10 --showalignment no
#~/sharedscratch/apps/exonerate -q cel_sl.fa -t $outdir/tails.rev.fa -n 1 --score 10 --showalignment no
~/sharedscratch/apps/exonerate -q cel_sl.fa -t $outdir/tails.all.fa -n 1 --score 10 --showalignment no

#### only keep mapped alignments
mbam=${bam/.bam/.mapped.bam}
#samtools view -F 4 -hb -@ $threads $bam > $mbam
...
# Identify candidate spliced leaders
echo "Identifting candidate spliced leaders ..."
Rscript slidr_analyse.R $outdir $threads
...
# index
srun -c 12 --mem 12G hisat2-build -p 12 trichinella_spiralis.PRJNA12603.WBPS10.genomic.fa tsp_genome

# align
srun -c 16 --mem 32G hisat2 --rna-strandness unstranded --sp 1,1 -x tsp_genome -1 888*_val_1.fq.gz -2 888*_val_2.fq.gz -p 16 | samtools sort -@ 16 -O bam -o hisat.sp1_1.out.bam -

srun -c 16 --mem 32G hisat2 --rna-strandness unstranded --sp 1,1 -x cel_genome -1 SRR.R1.fq.gz -2 SRR.R2.fq.gz -p 16 | samtools sort -@ 16 -O bam -o cel.hisat.sp1_1.out.bam -

srun -c 16 --mem 32G hisat2 --sp 1,1 -x cel_genome -1 SRR1585277*_1*gz -2 SRR1585277*_2*gz -p 16 | samtools sort -@ 16 -O bam -o SRR1585277.hisat.sp1_1.out.bam -

srun -c 16 --mem 32G hisat2 --rna-strandness RF --sp 1,1 -x cel_genome -1 ERR1474711*_1*gz -2 ERR1474711*_2*gz -p 16 | samtools sort -@ 16 -O bam -o ERR1474711.hisat.sp1_1.out.bam -


# A) hisat2 tail extraction
#########################################

# extract tails from soft-clipped reads
# reads mapping to forward strand can be taken as is (CIGAR e.g. 10S90M)
samtools view -F 20 SRR1585277.hisat.sp1_1.out.bam | awk '$6 ~ "^[0-9]*S"{gsub("S.*", "", $6); t=substr($10,1,$6); print ">"$1":::"$3":::"$4"\n"t}' > SRR1585277.tails.fwd.fa
# reads mapping to reverse strand must be reverse complemented (CIGAR e.g. 90M10S)
samtools view -F 4 -f 16 SRR1585277.hisat.sp1_1.out.bam | awk '$6 ~ "[0-9]*S$"{gsub("S", "", $6); gsub(".*[A-Z]", "", $6); t=substr($10,length($10)+1-$6,$6); print ">"$1":::"$3":::"$4"\n"t}' | seqtk seq -r > SRR1585277.tails.rev.fa

~/sharedscratch/apps/exonerate -q cel_sl.fa -t SRR1585277.tails.rev.fa -n 1 --score 10 --showalignment no
...
#samtools view -F 4 -f 64 hisat.sp1_1.out.bam | awk '$6 ~ "^[0-9]*S"' > softclipped.sam
#samtools view -F 4 -f 128 hisat.sp1_1.out.bam | awk '$6 ~ "^[0-9]*S"' > softclipped.sam

awk '{print ">"$1":::"$3":::"$4"\n"$10}' softclipped.sam > softclipped.fa


# extract tail lengths
awk '$6 ~ "^[0-9]*S"{gsub("S.*", "", $6); print $6}' softclipped.sam | sort | uniq -c | sort -rg -k2,2

# extract tail sequences
awk '$6 ~ "^[0-9]*S"{gsub("S.*", "", $6); t=substr($10,1,$6);print ">"$1"\n"t}' softclipped.sam > tails.fa


samtools view softclipped.sam | awk '$6 ~ "^[0-9]*S"{gsub("S.*", "", $6); print $6}'
...
srun makeblastdb -dbtype nucl -in trichinella_spiralis.PRJNA12603.WBPS10.genomic.fa -out tspgenome

# blast softclipped
sbatch -c 16 --mem 32G --wrap "blastn -query softclipped.fa -db tspgenome -word_size 8 -ungapped -num_threads 16 -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq' | gzip > softclipped.fa.blastn.out.gz"

# blast all
#sbatch -c 16 --mem 32G -t 96:00:00 -p uoa-gpu --wrap "seqtk seq -A 888_*.fq.gz | blastn -query - -db tspgenome -word_size 8 -ungapped -num_threads 16 -outfmt '6 qseqid #sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq' | gzip > blastn.out.gz"

# analyse BLAST output
# best tails
zcat softclipped.fa.blastn.out.gz | awk '$7<3 && $3=="100.000" && $4>15 && $4<25' | sort -k1,1 -k12,12 -r | sort -u -k1,1
...
~/sharedscratch/apps/utils/samtools view bbnormecc_100.spades.fa.bam.mapped \
	| awk '$6 ~ "^17S|^18S|^19S|^20S"{gsub("S.*", "", $6); t=substr($10,1,$6);print t}'

 awk '$6 ~ /^19S/{t=substr($10,1,20);print t}' hisat.out.bam.mapped.sam | sort | uniq -c | sort -rn | head
 
echo "hisat2 --sp 0,0 -x platanus_full.fa -1 ../clean/e_brevis/ERR660661_1.fastq.bz2_val_1.fq.gz -2 ../clean/e_brevis/ERR660661_2.fastq.bz2_val_2.fq.gz -p 16 | samtools sort -@ 16 -O bam -o hisat.sp0_0.out.bam" | qsub -V -cwd -N hisat -pe smp 16


for a in ../genome/e_brevis/spades/bbnormecc_100.spades.fa
do
echo "hisat2 --sp 1,0 -x $a -1 ../clean/e_brevis/ERR660661_1.fastq.bz2_val_1.fq.gz -2 ../clean/e_brevis/ERR660661_2.fastq.bz2_val_2.fq.gz -p 16 | samtools sort -@ 16 -O bam -o $a.bam" | qsub -V -cwd -N hisat -pe smp 16
done

########## strategy 2: use known conserved downstream genes to do a targetted search
# get Cel proteins for the genes
~/apps/ncbi-blast-2.6.0+/bin/blastx -db ../trichinella/synteny/Cel_prot -query nematode_operons.fa -outfmt 6 | sort -k1,1 -k11g,11 | sort -u -k1,1 | cut -f 2 > nematode_operons.fa.Cel_prot
~/apps/seqtk-master/seqtk seq -l 0 ../trichinella/genome/caenorhabditis_elegans.PRJNA13758.WBPS14.protein.fa | grep --no-group-separator -A1 -Fw -f nematode_operons.fa.Cel_prot > nematode_operons.fa.Cel_prot.fa
~/sharedscratch/apps/utils/exonerate -q nematode_operons.fa.Cel_prot.fa -t genome/e_brevis/spades/bbnormecc_100.spades.fa -n 1 --showalignment no -m p2g
~/sharedscratch/apps/utils/samtools view SL_screen_genome/bbnormecc_100.spades.fa.bam.mapped | grep -Fw -f <( awk '/NODE/{print $6}' nematode_operons.fa.Cel_prot.fa.bbnormecc_100.spades.fa.exonerate) > candidates.sam
